...
// --- 1. Extract helmet <style> CSS (strip @font-face; we load fonts from Google Fonts) ---
const styleMatch = html.match(/<helmet>[\s\S]*?<style>([\s\S]*?)<\/style>/);
let css = styleMatch ? styleMatch[1] : '';
css = css.replace(/@font-face\s*\{[^}]*\}/g, '').trim();

// --- 2. Isolate the pages (drop DC widget, wrappers, and trailing runtime script) ---
...
pages = pages.replace(/\{\{\s*([a-zA-Z0-9_]+)\s*\}\}/g, (m, key) => {
  if (Object.prototype.hasOwnProperty.call(bind, key)) return bind[key];
  return ''; // drop any stray on* handler tokens etc.
});

// --- 4. Replace <image-slot> face placeholders with initials avatars ---
...
// --- 5. Replace AI Central logo images with a clean SVG wordmark (keeps height styling) ---
function wordmark(ink) {
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 150 22">` +
    `<text x="0" y="17" font-family="Montserrat,Arial,sans-serif" font-size="18" font-weight="700" ` +
    `fill="${ink}" letter-spacing="0.4">AI CENTRAL</text></svg>`;
  return 'data:image/svg+xml,' + encodeURIComponent(svg);
}
pages = pages.replace(/src="assets\/logo-full-light-bg\.png"/g, `src="${wordmark('#111111')}"`);
pages = pages.replace(/src="assets\/logo-full-dark-bg\.png"/g, `src="${wordmark('#FFFDFA')}"`);

// --- 6. Remove paper-texture background overlays (asset not bundled) ---
pages = pages.replace(/url\('assets\/bg-paper-texture-default\.png'\)\s*center\/cover/g, 'none');

// --- 7. Assemble final document ---
const CONFIG = { url: SUPABASE_URL, anon: SUPABASE_ANON, docId: DOC_ID };

const editorCSS = `
/* ---- editor chrome ---- */
#wp-bar{position:fixed;top:0;left:0;right:0;z-index:1000;display:flex;align-items:center;gap:14px;
  padding:9px 16px;background:#111111;color:#FFFDFA;font-family:'Inter',sans-serif;font-size:13px;
...
#wp-bar button{font-family:'Inter',sans-serif;font-size:12.5px;font-weight:600;cursor:pointer;
  border:1.5px solid #FED600;background:#FED600;color:#111111;padding:6px 14px;border-radius:2px;transition:.15s}
#wp-bar button.ghost{background:transparent;color:#FFFDFA;border-color:#4A4A4A}
#wp-bar button.ghost:hover{border-color:#FFFDFA}
#wp-bar button:hover{filter:brightness(1.05)}
...
body.wp-editing .wp-hint{display:inline}
/* editable affordances only in edit mode */
...
body.wp-editing [data-eid]:focus{outline:2px solid #111111;background:rgba(254,214,0,.16)}
...
(function(){
  ...
  var PASSWORD_CACHE = null;
  var INLINE = {STRONG:1,EM:1,B:1,I:1,A:1,SPAN:1,BR:1,SUP:1,SUB:1,U:1,SMALL:1,MARK:1,CODE:1};
  ...
  function saveContent(map, password){
    return fetch(CFG.url+'/rest/v1/rpc/gta_whitepaper_save',
      {method:'POST',headers:headers({'Content-Type':'application/json'}),
       body:JSON.stringify({p_id:CFG.docId,p_content:map,p_password:password})})
      .then(function(r){
  ...
  // ---- UI ----
  var statusEl, editBtn, saveBtn, exitBtn, pwWrap, pwInput, pwGo;
  function setStatus(txt, cls){ statusEl.textContent = txt; statusEl.className=''; if(cls) statusEl.classList.add(cls); }

  function fmtTime(iso){
    if(!iso) return '';
    try{ var d=new Date(iso); return d.toLocaleString(undefined,{month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'}); }
    catch(e){ return ''; }
  }
  ...
  function enterEdit(){
    editing=true;
    document.body.classList.add('wp-editing');
  ...
  function exitEdit(){
    editing=false;
    document.body.classList.remove('wp-editing');
    for(var i=0;i<editables.length;i++){ editables[i].removeAttribute('contenteditable'); }
    saveBtn.style.display='none'; exitBtn.style.display='none';
    editBtn.style.display='';
    setStatus(lastSaved?('Viewing - last updated '+lastSaved):'Viewing', '');
  }

  function doSave(){
    if(!PASSWORD_CACHE) return;
    var map = collectOverrides();
    setStatus('Saving...','saving');
    saveContent(map, PASSWORD_CACHE).then(function(res){
  ...
  function tryUnlock(){
    ...
    // validate by attempting a no-op save of current overrides
    saveContent(collectOverrides(), pw).then(function(res){
      if(res.ok){ PASSWORD_CACHE=pw; enterEdit(); }
      else{ setStatus('Wrong password','err'); pwInput.select(); }
    });
  }
  ...
  function buildBar(){
    ...
    bar.innerHTML =
      ...
      '<span id="wp-status">Loading...</span>'+
    ...
    editBtn=document.getElementById('wp-edit');
    saveBtn=document.getElementById('wp-save');
    exitBtn=document.getElementById('wp-exit');
    pwWrap=document.getElementById('wp-pw');
    pwInput=document.getElementById('wp-pw-input');
    pwGo=document.getElementById('wp-pw-go');

    editBtn.addEventListener('click', function(){
      pwWrap.style.display=(pwWrap.style.display==='none')?'':'none';
      if(pwWrap.style.display!=='none') pwInput.focus();
    });
...
// sanity checks
const leftoverTokens = (out.match(/\{\{[^}]*\}\}/g) || []);
const leftoverSlots = (out.match(/<image-slot/g) || []);
const leftoverAssets = (out.match(/assets\//g) || []);
console.log('Wrote public/index.html', out.length, 'bytes');
console.log('leftover {{tokens}}:', leftoverTokens.length);
console.log('leftover <image-slot>:', leftoverSlots.length);
console.log('leftover assets/ refs:', leftoverAssets.length);
console.log('pages:', (out.match(/<section class="page"/g) || []).length);
